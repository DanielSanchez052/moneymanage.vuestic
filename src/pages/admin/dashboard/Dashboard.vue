...
const sourcesPerMonth = computed(() => months
  .map((month: string, idx: number): SourceTotalPerMonth => ({
    month,
    total: sources.value
      .filter(f => transactions.value.items.some(t => t.source.id == f.id && new Date(t.transactionDate).getMonth() == idx))
      .map(s => ({
        sourceId: s?.id ?? 0,
        sourceName: s?.name ?? "",
        earning: transactions.value.items
          .filter((t: Transaction) => new Date(t.transactionDate).getMonth() == idx && t.source.id == s.id && t.type.id == 1)
          .reduce((n: number, { ammount }: Transaction) => n + ammount, 0),
        expenses: transactions.value.items
          .filter((t: Transaction) => new Date(t.transactionDate).getMonth() == idx && t.source.id == s.id && t.type.id == 2)
          .reduce((n: number, { ammount }: Transaction) => n + ammount, 0),
      }))
    })
  )
...
const sourcesUsage = computed(() : SourceTotal[] => {
  const grouped = Object.groupBy(sourcesPerMonth.value.flatMap(s => s.total), ({sourceId}) => sourceId)
  return sources.value.map((s) : SourceTotal => ({
    sourceId: s.id,
    sourceName: s.name ?? "",
    earning: Object.keys(grouped).some(k => k == s.id) ? grouped[s.id]?.reduce((n: number,{ earning }: SourceTotal) => n+earning, 0) ?? 0 : 0,
    expenses:  Object.keys(grouped).some(k => k == s.id) ? grouped[s.id]?.reduce((n: number,{ expenses}: SourceTotal) => n+expenses, 0) ?? 0 : 0
  }))
})

const earningBigger = computed(() => sourcesUsage.value.sort((a, b) => a.earning - b.earning).reverse()[0])
const expenseBigger = computed(() => sourcesUsage.value.sort((a, b) => a.expenses - b.expenses).reverse()[0])

const earningsOfMostBiggerSource = computed(() => months.map((month: string, idx: number) =>
  transactions.value.items
    .filter((t: Transaction) => new Date(t.transactionDate).getMonth() == idx && t.source.id == expenseBigger.value.sourceId && t.type.id == 2)
    .reduce((n: number, { ammount }: Transaction) => n + ammount, 0)
))

const expensesOfMostBiggerSource = computed(() => months.map((month: string, idx: number) =>
  transactions.value.items
    .filter((t: Transaction) => new Date(t.transactionDate).getMonth() == idx && t.source.id == earningBigger.value.sourceId && t.type.id == 1)
    .reduce((n: number, { ammount }: Transaction) => n + ammount, 0)
))
...
<template>
  ...
  <section class="flex flex-col gap-4">
    ...
    <div class="flex flex-col sm:flex-row gap-4" v-if="!isLoading">
    ...
    <div class="flex flex-col sm:flex-row gap-4" v-if="!isLoading && !sourceIsLoading">
      <div class="flex flex-col gap-4 w-full sm:w-[30%]">
        <MonthlyChart :transactions="earningsOfMostBiggerSource" chart-background="rgba(75,192,192,0.4)" :chart-label="`Earnings ${earningBigger.sourceName}`" />
        <MonthlyChart :transactions="expensesOfMostBiggerSource" chart-background="rgba(75,192,192,0.4)" :chart-label="`Expenses ${expenseBigger.sourceName}`" />
      </div>
